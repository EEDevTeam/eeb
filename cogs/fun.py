...
responses = ['Ja', 'Nein', 'Vieleicht', 'Ja, auf jeden Fall', 'Frag mich nochmal', 'Frag mich später']
...
class fun(commands.Cog):
    ...
    async def java(self, ctx):
        '''Weil Java != Javscript'''
        await ctx.send(
            ':interrobang: Meintest du jQuery, Javascript oder Node.js? https://abload.de/img/2016-05-102130191kzpu.png')
    ...
    async def random(self, ctx, *arg):
        ...
        if ctx.invoked_subcommand is None:
            ...
            await ctx.send(
                f'**:arrows_counterclockwise:** Zufällige Zahl ({start} - {end}): {random.randint(start, end)}')
    ...
    async def _8ball(self, ctx, *, question):
        ...
        embed.add_field(name="Frage:ㅤㅤㅤ", value=question)
        embed.add_field(name="Anwort:ㅤㅤㅤ", value=random.choice(responses))
        ...
        with sqlite3.connect(self.db) as con:
            ...
            if command == 'add' or command == 'new':
                if len(arg) > 1:
                    command = arg[0].lower()
                    content = list(arg[1:])
                    c.execute('INSERT INTO "reactions" ("command","url","author") VALUES (?, ?, ?)',
                              (command, ' '.join(content), str(ctx.message.author)))
                    con.commit()
                ...
                if await ctx.bot.is_owner(ctx.author):
                    ...
                    c.execute('DELETE FROM "reactions" WHERE "id" in (?) AND "author" IN (?)',
                              (int(arg[0]), str(ctx.message.author)))
                con.commit()
